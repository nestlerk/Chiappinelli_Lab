...
- add functionality for single strand and for unstranded data
...
assert len(controlSample) > 0, "control_sample not set in config file!"
assert len(working_dir) > 0, "working_dir not set in config file!"
assert len(raw_file_ext) > 0, "raw_file_extension not set in config file!"
assert len(replicates) > 0, "num_replicates not set in config file!"

#run test on file naming convention
assert raw_file_ext == 'fastq.gz' or raw_file_ext == 'fq.gz', "raw_file_extension must be fastq.gz or fq.gz!"
...
#add some multiqc stuff here in the future
...
rule STAR:
	...
	shell:
		'''
		ml star/2.7.0e
		STAR --runThreadN 16 --genomeDir /SMHS/groups/chiappinellilab/genomes/hg38/STAR.hg38.index.2.7.0e/ \
		--sjdbGTFfile /SMHS/groups/chiappinellilab/genomes/hg38/gencode.annotation/gencode.v21.primary.assembly.only.annotation.gtf \
		--sjdbOverhang 100 \
...
# do we want to add a rule for TEcount?
# rule TEcount:
# 	input:
# 		working_dir + 'RNAseq.STAR/RNAseq.STAR.{sample}_{replicate}.Aligned.out.bam'
# 	output:
# 		working_dir + 'TEcount/{sample}_{replicate}.TEcount.txt'
# 	shell:
